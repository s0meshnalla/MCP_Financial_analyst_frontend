...
const GraphsPage = () => {
  ...
  // Date range filtering for stock prices — default to last 30 days
  ...
  const [dateStartInput, setDateStartInput] = useState(defaultStart);
  const [dateEndInput, setDateEndInput] = useState(defaultEnd);
  const [activeDateStart, setActiveDateStart] = useState(defaultStart);
  const [activeDateEnd, setActiveDateEnd] = useState(defaultEnd);
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      ...
      setError(null);
      try {
        const params = { symbol: selectedTicker.symbol };
        if (activeDateStart && activeDateEnd) {
          params.startDate = activeDateStart;
          params.endDate = activeDateEnd;
        }
        const res = await api.get('/stockdailyprices', { params });
    ...
    fetchPrices();
  }, [selectedTicker, activeDateStart, activeDateEnd]);
              ...
              <button
                onClick={() => {
                  if (dateStartInput && dateEndInput) {
                    setActiveDateStart(dateStartInput);
                    setActiveDateEnd(dateEndInput);
                  }
                }}
                disabled={!dateStartInput || !dateEndInput}
                className="px-2 py-0.5 rounded-md text-[11px] font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition"
              >
                Apply
              </button>
              {activeDateStart && (
                <button
                  ...
                  className="p-0.5 rounded-md text-gray-400 hover:bg-gray-100 transition"
